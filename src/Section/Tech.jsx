        ...
        {technologies.map((tech) => (
          <div key={tech.name} className="flex flex-col items-center gap-3">
            <div
              ...
              onMouseEnter={(e) => {
                e.currentTarget.style.boxShadow = `0 10px 40px -5px ${tech.color}66`;
            ...
            <p className="text-zinc-400 text-sm font-medium font-sora tracking-wide uppercase opacity-0 group-hover:opacity-100 transition-opacity duration-300">
              {tech.name}
            </p>
            <span className="text-zinc-300 text-sm font-semibold font-sora mt-1">
              {tech.name}
            </span>
          </div>
        ))}
